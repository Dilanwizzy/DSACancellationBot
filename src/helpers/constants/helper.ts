...
export function pickTheBestDate(bookedTimeDto: BookedTimeDto, newDate: Date): Date {
    ...
    const timeDiff =  (moment(new Date(newDate)).valueOf() - moment(new Date(prevTime)).valueOf()) / 60000;
    ...
    if(timeDiffFromEarliestTime > 0 ) {
        ...
        if(timeDiff > 21000 && prevTime.getHours() < 11 && newDate.getHours() >= 11) {
            ...
            if((timeDiff < 21000 && timeDiff > -21000) && prevTime.getHours() < 11 && newDate.getHours() >= 11) {
                //Pick new time 
                return newDate
            } else {
                return prevTime;
